...
class DataGuidePopup(QDialog):
    ...
    def _setup_ui(self):
        ...
        def create_card(title, widget_items):
            card = QFrame()
            card.setStyleSheet("QFrame { background-color: white; border: 1px solid #E5E7EB; border-radius: 10px; padding: 2px; }")
        ...
        # 1. 지원 파일
        sec1_lbl1 = make_lbl("GichanFormant는 <b>.txt, .xlsx, .csv, .tsv</b> 파일을 지원합니다.")
        sec1_lbl2 = make_lbl("여러 파일을 동시에 클릭 또는 드래그 앤 드롭으로 로드할 수 있습니다.")
        sec1_lbl3 = make_lbl("(단, 한 번에 수백 개의 파일을 무리하게 넣으면 성능이 저하될 수 있습니다.)", "#666666", 9)

        content_layout.addWidget(create_card("1. 지원 파일 및 로드 가이드", [sec1_lbl1, sec1_lbl2, sec1_lbl3]))

        # 2. 열 구성 및 예외 처리
        sec2_lbl1 = make_lbl("• 1열 (A열): <b>F1</b>")
        sec2_lbl2 = make_lbl("• 2열 (B열): <b>F2</b>")
        sec2_lbl3 = make_lbl("• 3열 (C열): <b>F3</b> (선택), 그 다음 열: <b>라벨</b>. F1~F3까지만 포먼트로 인식하며 F4부터는 지원하지 않습니다.")
        sec2_lbl4 = make_lbl("- 첫 번째 줄(1행)에 헤더(예: \"F1\", \"F2\" 등)가 있어도 자동으로 무시됩니다.", "#666666", 9)
        ...
        sec2_lbl5 = make_lbl("- 데이터의 소수점은 계산 시 자동으로 반올림 처리됩니다.", "#666666", 9)
        sec2_lbl6 = make_lbl("- F1이 F2보다 큰 논리적 오류 데이터는 자동으로 제외됩니다.", "#666666", 9)

        content_layout.addWidget(create_card("2. 열(Column) 구성 및 처리 규칙",
                                             [sec2_lbl1, sec2_lbl2, sec2_lbl3, sec2_lbl4, sec2_table, sec2_lbl5,
                                              sec2_lbl6]))

        # 3. 모음 기호 규칙
        sec3_lbl1 = make_lbl("• 모음 기호는 반드시 <b style='color: #1976d2;'>슬래시 / /</b> 기호로 감싸야 합니다.")
        ...
        ex_bad = make_lbl("<b><span style='color: #d32f2f;'>X 잘못된 예:</span></b>&nbsp;&nbsp;&nbsp;<b>a, i, u, ㅏ, ʌ, \"e\", [ㅜ]</b>", size=11)
        ex_good = make_lbl("<b><span style='color: #1976d2;'>O 올바른 예:</span></b>&nbsp;&nbsp;&nbsp;<b>/a/, /i/, /u/, /ㅏ/, /ʌ/, /aː/</b> (ː=장음 기호)", size=11)
        ex_layout.addWidget(ex_bad)
        ex_layout.addWidget(ex_good)

        sec3_lbl2 = make_lbl("- 기호는 로마자, 한글, IPA, 장음 기호(ː) 등 모두 사용 가능합니다.", "#666666", 9)
        sec3_lbl3 = make_lbl("- 슬래시가 없는 데이터는 분석에서 제외됩니다.", "#666666", 9)

        content_layout.addWidget(create_card("3. 모음 기호(Label) 규칙", [sec3_lbl1, ex_box, sec3_lbl2, sec3_lbl3]))
